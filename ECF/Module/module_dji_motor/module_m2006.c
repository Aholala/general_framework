                                        ...
                                        const module_m2006_config_t *const config)
{
    ...
    // 参数校验
    if ((me == NULL) || (config == NULL) || (config->control_mode > MODULE_M2006_CONTROL_ANGLE))
    {
        return MODULE_MOTOR_STATUS_INVALID_ARGUMENT;
    }
                                                          ...
                                                          int16_t command_raw)
{
    module_motor_status_t status =
        module_m2006_validate_control_mode(me, MODULE_M2006_CONTROL_DIRECT);
    if (status != MODULE_MOTOR_STATUS_OK)
    {
        return status;
    }
    // DIRECT 模式的目标值直接作为命令值（原始电流）
    return module_motor_set_target(module_m2006_as_motor(me), (float)command_raw);
}
...
module_motor_status_t module_m2006_set_current_a(module_m2006_t *const me, float current_a)
{
    ...
    return (status == MODULE_MOTOR_STATUS_OK)
               ? module_motor_set_target(module_m2006_as_motor(me), current_a)
               : status;
}
...
{
    module_motor_status_t status =
        module_m2006_validate_control_mode(me, MODULE_M2006_CONTROL_ANGLE);
    if (status != MODULE_MOTOR_STATUS_OK)
    {
        return status;
    }
    return module_motor_set_target(module_m2006_as_motor(me), angle_rad);
}
