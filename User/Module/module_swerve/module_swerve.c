...
static module_swerve_status_t module_swerve_read_steering_angle(
    const module_swerve_t *me, float *steering_angle_rad)
{
    ...
    *steering_angle_rad =
        (steering_feedback->position_rad * me->steering_direction_sign) -
    ...
    module_swerve_t *me, const module_swerve_config_t *config)
{
    if ((me == NULL) || (config == NULL) ||
        (config->drive_motor == NULL) ||
        (config->steering_motor == NULL) ||
        !config->drive_motor->is_initialized ||
        !config->steering_motor->is_initialized ||
        !isfinite(config->wheel_radius_m) ||
        (config->wheel_radius_m <= 0.0F) ||
        !isfinite(config->drive_reduction_ratio) ||
        (config->drive_reduction_ratio <= 0.0F) ||
        !isfinite(config->steering_zero_offset_rad) ||
        ((config->drive_direction_sign != 1.0F) &&
         (config->drive_direction_sign != -1.0F)) ||
        ((config->steering_direction_sign != 1.0F) &&
         (config->steering_direction_sign != -1.0F)))
    {
...
module_swerve_status_t module_swerve_disable(module_swerve_t *me)
{
    ...
    me->is_enabled = false;
    return ((drive_status == MODULE_MOTOR_STATUS_OK) &&
            (steering_status == MODULE_MOTOR_STATUS_OK))
    ...
    float delta_time_s)
{
    ...
    if ((me == NULL) || (target == NULL) ||
        !isfinite(target->wheel_velocity_m_per_s) ||
    ...
    status = module_swerve_read_steering_angle(
        me, &current_steering_angle_rad);
    if (status != MODULE_SWERVE_STATUS_OK)
    {
        return status;
    }
    optimized_target = *target;
    if (alg_swerve_optimize_target(current_steering_angle_rad,
                                   &optimized_target) !=
        ALG_SWERVE_STATUS_OK)
    {
        return MODULE_SWERVE_STATUS_ALGORITHM_ERROR;
    }
    drive_velocity_rad_per_s =
        optimized_target.wheel_velocity_m_per_s / me->wheel_radius_m *
        me->drive_reduction_ratio * me->drive_direction_sign;
    steering_target_rad =
        (optimized_target.steering_angle_rad +
         me->steering_zero_offset_rad) *
        me->steering_direction_sign;
    if ((module_motor_set_target(me->drive_motor,
                                 drive_velocity_rad_per_s) !=
         MODULE_MOTOR_STATUS_OK) ||
        (module_motor_set_target(me->steering_motor,
                                 steering_target_rad) !=
         MODULE_MOTOR_STATUS_OK) ||
        (module_motor_update(me->drive_motor, delta_time_s) !=
         MODULE_MOTOR_STATUS_OK) ||
        (module_motor_update(me->steering_motor, delta_time_s) !=
         MODULE_MOTOR_STATUS_OK))
    {
        return MODULE_SWERVE_STATUS_MOTOR_ERROR;
    }
