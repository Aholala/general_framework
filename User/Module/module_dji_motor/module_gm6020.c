/**
 * @file module_gm6020.c
 * @author Ahola邱泽钦 (aholace0328@gmail.com)
 * @brief 大疆 GM6020 云台电机专用派生模块实现
 * @version 1.0
 * @date 2026-07-28
...
/**
 * @brief 校验 GM6020 的控制模式是否匹配
 * @param me 电机对象
 * @param expected_control_mode 期望的控制模式
 * @return 执行状态
 * @note 若控制模式不匹配，返回 UNSUPPORTED，防止角度误当速度或物理电压误当协议命令
 */
static module_motor_status_t
module_gm6020_validate_control_mode(const module_gm6020_t *const me,
                                    module_gm6020_control_mode_t expected_control_mode)
{
    if (me == NULL)
    {
    ...
    // 检查当前配置的控制模式是否与期望一致
    if (me->super.control_mode != module_gm6020_map_control_mode(expected_control_mode))
    {
 ...
 * @param config 配置参数
 * @return 执行状态
 * @note 构建 module_dji_motor_config_t，固定 motor_model = MODULE_DJI_MOTOR_GM6020
 *       GM6020 减速比固定为 1.0F（直驱），不支持更改
 */
                                         ...
                                         const module_gm6020_config_t *const config)
{
    ...
    dji_motor_config = (module_dji_motor_config_t){
        ...
        .control_mode = module_gm6020_map_control_mode(config->control_mode),
...
{
    return module_motor_enable(module_gm6020_as_motor(me));
}
...
{
    return module_motor_disable(module_gm6020_as_motor(me));
}
                                                            ...
                                                            int16_t voltage_command_raw)
{
    module_motor_status_t status =
        module_gm6020_validate_control_mode(me, MODULE_GM6020_CONTROL_VOLTAGE);
    if (status != MODULE_MOTOR_STATUS_OK)
    {
        return status;
    }
    // DIRECT 模式的目标值直接作为命令值（原始电压）
    return module_motor_set_target(module_gm6020_as_motor(me), (float)voltage_command_raw);
}
...
{
    module_motor_status_t status =
        module_gm6020_validate_control_mode(me, MODULE_GM6020_CONTROL_VELOCITY);
    if (status != MODULE_MOTOR_STATUS_OK)
    {
        return status;
    }
    return module_motor_set_target(module_gm6020_as_motor(me), velocity_rad_per_s);
}
...
{
    return module_motor_update(module_gm6020_as_motor(me), delta_time_s);
}
