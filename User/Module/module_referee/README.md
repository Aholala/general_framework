...
`module_referee_data.h` 提供比赛状态、机器人状态、底盘功率与枪口热量、位置、受击、射击和剩余弹量等常用命令的强类型结构。把
`module_referee_data_route_handler` 作为默认路由或对应命令路由即可完成解码；更新位图允许控制任务只消费本周期的新数据。

协议每赛季可能调整。底层帧解析、CRC 和路由保持稳定；赛季变更只需核对命令 ID、载荷长度和字段定义，不要把官方 packed 结构直接强制转换到接收缓冲区。
