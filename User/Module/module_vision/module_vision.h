...
/** @brief 帧头第一个字节（0xA5） */
#define MODULE_VISION_FRAME_HEADER_FIRST (0xA5U)
/** @brief 帧头第二个字节（0x5A） */
#define MODULE_VISION_FRAME_HEADER_SECOND (0x5AU)
/** @brief 帧总大小（5 字节） */
#define MODULE_VISION_FRAME_SIZE (5U)
/** @brief CRC8 输入大小（帧头 2 + 数据 2 = 4 字节） */
#define MODULE_VISION_CRC_INPUT_SIZE (4U)
    ...
    typedef enum
    {
        MODULE_VISION_STATUS_OK = 0,           // 操作成功
        MODULE_VISION_STATUS_INVALID_ARGUMENT, // 参数非法
        MODULE_VISION_STATUS_NOT_INITIALIZED,  // 对象未初始化
        MODULE_VISION_STATUS_TRANSPORT_ERROR,  // USB 传输错误
        MODULE_VISION_STATUS_BUSY,             // USB 发送忙
        MODULE_VISION_STATUS_INVALID_FRAME,    // 收到无效帧（CRC 错误等）
        MODULE_VISION_STATUS_NO_DATA           // 没有有效数据（未收到帧）
    ...
    typedef struct
    {
        bsp_usb_vcp_t *usb_vcp;                            // USB VCP BSP 基类
        uint32_t transmit_timeout_ms;                      // 发送超时（毫秒）
        uint8_t transmit_buffer[MODULE_VISION_FRAME_SIZE]; // 发送缓冲区
        uint8_t stream_buffer[MODULE_VISION_FRAME_SIZE];   // 流解析缓冲区
        size_t stream_size;                                // 流缓冲区有效字节数
        module_vision_data_t received_data;                // 最新接收数据
