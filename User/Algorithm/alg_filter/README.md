...
1. 根据噪声类型选择一种滤波器，不要把所有滤波器串联当作默认方案。
2. 静态创建对应 `alg_filter_*_t`；移动平均、Median、FIR 还要准备调用者拥有的窗口/历史数组。
3. 调用对应 `*_init()`，确认采样周期、截止频率、权重或系数合法。
4. 每个采样点只调用一次 `*_update()`，其返回值就是本周期滤波输出。
5. 采样间断、传感器重连或模式切换时调用对应 `*_reset()`；在线改参数使用 `set_*()`。

| 可读取结构体                               | 主要信息                     |
| ------------------------------------------ | ---------------------------- |
| `alg_filter_low_pass_t` / `high_pass_t`    | 系数、上次输入和当前输出     |
| `alg_filter_moving_average_t` / `median_t` | 窗口、索引、有效样本数和输出 |
| `alg_filter_fir_t`                         | 系数、历史样本和环形索引     |
| `alg_filter_biquad_t`                      | 二阶系数、延迟状态和输出     |
| `alg_filter_complementary_t`               | 两路输入权重和融合结果       |

对象字段可以用于调试滤波状态，但应用应使用 update 返回值，不要直接修改历史样本或内部延迟量。
