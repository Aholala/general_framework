                                            ...
                                            const alg_chassis_fault_config_t *config)
{
    ...
    {
        me->wheel_states[wheel_index] = (alg_chassis_fault_wheel_state_t){0};
    }
                                              ...
                                              bool *wheel_is_available, size_t output_capacity)
{
    ...
    for (wheel_index = 0U; wheel_index < me->wheel_count; ++wheel_index)
    {
        alg_chassis_fault_wheel_state_t *const wheel_state = &me->wheel_states[wheel_index];
        ...
        else if (!wheel_state->is_faulted)
        {
            ...
            if (residual_m_per_s >= me->fault_residual_threshold_m_per_s)
            {
                wheel_state->fault_confirmation_count =
                    alg_chassis_fault_increment_counter(wheel_state->fault_confirmation_count);
        ...
        else
        {
            ...
            if (residual_m_per_s <= me->recovery_residual_threshold_m_per_s)
            {
                wheel_state->recovery_confirmation_count =
                    alg_chassis_fault_increment_counter(wheel_state->recovery_confirmation_count);
                                                   ...
                                                   bool assume_available)
{
    ...
    me->wheel_states[wheel_index] = (alg_chassis_fault_wheel_state_t){
