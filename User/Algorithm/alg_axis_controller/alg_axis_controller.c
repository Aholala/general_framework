...
alg_axis_pid_reset_virtual(alg_axis_controller_t *controller_base,
                           float measured_position_rad,
                           float measured_velocity_rad_per_s, float initial_output)
{
    alg_axis_pid_t *const me = ALG_AXIS_CONTROLLER_CONTAINER_OF(
        controller_base, alg_axis_pid_t, super);
    const alg_pid_status_t status =
        alg_pid_cascade_reset(&me->cascade, measured_position_rad,
...
static alg_axis_controller_status_t
alg_axis_lqr_reset_virtual(alg_axis_controller_t *controller_base,
                           float measured_position_rad,
                           float measured_velocity_rad_per_s, float initial_output)
{
    (void)controller_base;
    return (isfinite(measured_position_rad) &&
            isfinite(measured_velocity_rad_per_s) && isfinite(initial_output))
...
alg_axis_lqr_update_virtual(alg_axis_controller_t *controller_base,
                            ...
                            float *control_output)
{
    alg_axis_lqr_t *const me = ALG_AXIS_CONTROLLER_CONTAINER_OF(
        controller_base, alg_axis_lqr_t, super);
    const float state[2] = {input->measured_position_rad,
                            input->measured_velocity_rad_per_s};
    const float reference_state[2] = {input->target_position_rad,
                                      input->target_velocity_rad_per_s};
    const float feedforward_control[1] = {input->actuator_feedforward};
    const alg_lqr_status_t status = alg_lqr_controller_update(
        &me->controller, state, reference_state, &me->equilibrium_control,
...
alg_axis_pid_init(alg_axis_pid_t *me, const alg_axis_pid_config_t *config)
{
    ...
    if (alg_pid_cascade_init(&me->cascade, &config->cascade_config) !=
        ALG_PID_STATUS_OK)
    {
...
alg_axis_lqr_init(alg_axis_lqr_t *me, const alg_axis_lqr_config_t *config)
{
    ...
    if ((me == NULL) || (config == NULL) || (config->gain_matrix == NULL) ||
        ...
        !isfinite(config->equilibrium_control) ||
    ...
    if (alg_lqr_controller_init(&me->controller, &controller_config) !=
        ALG_LQR_STATUS_OK)
    {
        return ALG_AXIS_CONTROLLER_STATUS_ALGORITHM_ERROR;
    }
...
alg_axis_controller_update(alg_axis_controller_t *me,
                           ...
                           float *control_output)
{
    if ((me == NULL) || (control_output == NULL) ||
        !alg_axis_controller_input_is_valid(input))
    {
        return ALG_AXIS_CONTROLLER_STATUS_INVALID_ARGUMENT;
    }
