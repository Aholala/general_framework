...
{
    if ((module_geometry == NULL) || !isfinite(half_wheelbase_m) ||
        (half_wheelbase_m <= 0.0F) ||
        !isfinite(half_track_width_m) ||
        (half_track_width_m <= 0.0F))
    {
        return ALG_SWERVE_STATUS_INVALID_ARGUMENT;
    }
    module_geometry[ALG_SWERVE_MODULE_FRONT_LEFT] =
        (alg_swerve_module_geometry_t){
            .position_x_m = half_wheelbase_m,
            .position_y_m = half_track_width_m,
        };
    module_geometry[ALG_SWERVE_MODULE_FRONT_RIGHT] =
        (alg_swerve_module_geometry_t){
            .position_x_m = half_wheelbase_m,
            .position_y_m = -half_track_width_m,
        };
    module_geometry[ALG_SWERVE_MODULE_REAR_LEFT] =
        (alg_swerve_module_geometry_t){
            .position_x_m = -half_wheelbase_m,
            .position_y_m = half_track_width_m,
        };
    module_geometry[ALG_SWERVE_MODULE_REAR_RIGHT] =
        (alg_swerve_module_geometry_t){
            .position_x_m = -half_wheelbase_m,
            .position_y_m = -half_track_width_m,
        };
    ...
    size_t module_count, float maximum_wheel_velocity_m_per_s)
{
    ...
    if ((me == NULL) || (module_geometry == NULL) || (module_count == 0U) ||
        !isfinite(maximum_wheel_velocity_m_per_s) ||
        (maximum_wheel_velocity_m_per_s <= 0.0F))
    {
        return ALG_SWERVE_STATUS_INVALID_ARGUMENT;
    }
    ...
    me->maximum_wheel_velocity_m_per_s =
        maximum_wheel_velocity_m_per_s;
...
{
    return alg_swerve_calculate_with_availability(
        me, command, NULL, module_targets, target_capacity);
}
    ...
    alg_swerve_module_target_t *module_targets, size_t target_capacity)
{
    ...
    if ((me == NULL) || (command == NULL) || (module_targets == NULL) ||
        !isfinite(command->velocity_x_m_per_s) ||
        !isfinite(command->velocity_y_m_per_s) ||
        !isfinite(command->angular_velocity_rad_per_s) ||
        !isfinite(command->reference_heading_rad) ||
        !isfinite(command->center_of_rotation_x_m) ||
    ...
    if (command->command_is_reference_relative)
    {
        ...
        body_velocity_x_m_per_s =
            (heading_cosine * command->velocity_x_m_per_s) +
            (heading_sine * command->velocity_y_m_per_s);
        body_velocity_y_m_per_s =
            (-heading_sine * command->velocity_x_m_per_s) +
    ...
    for (module_index = 0U; module_index < me->module_count; ++module_index)
    {
        const bool is_available =
            (module_is_available == NULL) ||
            module_is_available[module_index];
        const float wheel_velocity_x_m_per_s =
            body_velocity_x_m_per_s -
            (command->angular_velocity_rad_per_s *
             (me->module_geometry[module_index].position_y_m -
              command->center_of_rotation_y_m));
        const float wheel_velocity_y_m_per_s =
            body_velocity_y_m_per_s +
            (command->angular_velocity_rad_per_s *
             (me->module_geometry[module_index].position_x_m -
              command->center_of_rotation_x_m));
        ...
        module_targets[module_index].steering_angle_rad =
            (is_available && (wheel_velocity_m_per_s > 0.0F))
                ? atan2f(wheel_velocity_y_m_per_s,
        ...
        if (is_available &&
            (wheel_velocity_m_per_s >
             maximum_calculated_velocity_m_per_s))
        {
            maximum_calculated_velocity_m_per_s =
                wheel_velocity_m_per_s;
        }
    }
    if (maximum_calculated_velocity_m_per_s >
        me->maximum_wheel_velocity_m_per_s)
    {
        scale = me->maximum_wheel_velocity_m_per_s /
                maximum_calculated_velocity_m_per_s;
    }
    for (module_index = 0U; module_index < me->module_count; ++module_index)
    {
    ...
    return (available_module_count < me->module_count)
    ...
    size_t workspace_capacity, alg_chassis_solution_t *solution)
{
    ...
    if ((me == NULL) || (measured_module_states == NULL) ||
    ...
    for (module_index = 0U; module_index < me->module_count;
         ++module_index)
    {
        const alg_swerve_module_geometry_t *const geometry =
            &me->module_geometry[module_index];
    ...
    return alg_chassis_solve_velocity(
        constraint_workspace, 2U * me->module_count,
    ...
    alg_swerve_module_target_t *module_target)
{
    ...
    if ((module_target == NULL) ||
        !isfinite(current_steering_angle_rad) ||
        !isfinite(module_target->wheel_velocity_m_per_s) ||
        !isfinite(module_target->steering_angle_rad))
    {
        return ALG_SWERVE_STATUS_INVALID_ARGUMENT;
    }
    angle_error_rad = alg_swerve_wrap_angle_rad(
        module_target->steering_angle_rad - current_steering_angle_rad);
    if (angle_error_rad > ALG_SWERVE_HALF_PI)
    {
        module_target->steering_angle_rad =
            alg_swerve_wrap_angle_rad(
                module_target->steering_angle_rad - ALG_SWERVE_PI);
        module_target->wheel_velocity_m_per_s =
            -module_target->wheel_velocity_m_per_s;
    }
    ...
    size_t target_capacity)
{
    ...
    for (module_index = 0U; module_index < me->module_count; ++module_index)
    {
        ...
        module_targets[module_index].steering_angle_rad = atan2f(
            -me->module_geometry[module_index].position_y_m,
            -me->module_geometry[module_index].position_x_m);
    }
    return ALG_SWERVE_STATUS_OK;
