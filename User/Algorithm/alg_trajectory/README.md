...
`alg_trajectory_group` 使用五次时间缩放让任意数量轴同时起步、同时停止。它根据每轴位移和各自速度/加速度/加加速度限制求共同持续时间，适合云台 pitch/yaw、四舵轮转向以及机械机构联动。

在线切换目标时以当前输出位置为新起点，所有存储由调用方提供。同步组以零端速度/加速度结束；需要带非零端速度的单轴连续规划时继续使用基础 `alg_trajectory_t`。
