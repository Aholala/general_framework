...
1. 填写 `alg_mecanum_config_t`：轮半径、四轮坐标/几何、辊子布局、最大轮速和权重。
2. 调用 `alg_mecanum_init()`，对象可长期复用。
3. 控制方向使用 `alg_mecanum_inverse()`；需要偏置旋转中心时使用 `inverse_with_center_of_rotation()`。
4. 把四轮目标交给电机控制，并保存实际反馈轮速。
5. 里程计方向调用 `alg_mecanum_forward()` 得到 `alg_chassis_solution_t`，再按时间积分位姿。

| 可读取结构体             | 主要信息                                                        |
| ------------------------ | --------------------------------------------------------------- |
| `alg_mecanum_t`          | 已计算的四轮约束、轮半径、速度上限和布局                        |
| 四轮角速度数组           | 逆解目标或正解输入，固定顺序由 `alg_mecanum_wheel_index_t` 定义 |
| `alg_chassis_solution_t` | 正解速度、残差、使用轮数和降级标志                              |
| `applied_scale`          | 轮速超限时对整车命令施加的统一缩放比例                          |

轮不可用数组必须与固定轮序一致；降级正解可以继续估计，但 App 仍需决定是否允许运动。
