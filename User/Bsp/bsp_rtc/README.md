...
- **跨复位时间戳**：模块日志可使用 RTC 做跨复位时间戳，便于日志分析；短周期性能测量可使用单调递增的 `bsp_dwt`，任务级超时使用 RTOS tick，不能使用可能被校时回拨的 RTC。
...
- **实时控制**：短周期测量使用 `bsp_dwt`，任务级调度和长超时使用 RTOS tick，不能使用可能被校时回拨的 RTC。
...
1. 平台配置 RTC 时钟源和备份域，实现 driver ops。
2. 调用 `bsp_rtc_init()`；首次启动或时间无效时由 App 决定是否 set_time。
3. 日志显示使用 `bsp_rtc_get_time()`，排序/存储使用 `get_unix_time()`。
4. 设置时间只在可信来源可用时执行，并记录时钟跳变。
5. 不再使用时通过基类 deinit 解除绑定。

`bsp_rtc_time_t` 可读取年、月、日、星期、时、分、秒；Unix 时间通过输出参数读取。RTC 表示墙上时间，不适合控制周期和超时；短周期测量使用 `bsp_dwt`，任务级超时使用 RTOS tick。
