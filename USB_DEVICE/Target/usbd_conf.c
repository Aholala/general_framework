...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  * application, it is advised to not enable this option unless required. */
  ...
  HAL_PCDEx_SetRxFiFo(&hpcd_USB_HS, 0x80);
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_HS, 0, 0x40);
  /* USER CODE END FiFo_Configuration */
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_HS, 1, 0x80);
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_HS, 2, 0x40);
  return USBD_OK;
}
